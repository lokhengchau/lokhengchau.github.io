...
The original dataset contains the average college tuition cost, based on in-state tuition and fees for one year of full-time study at public four-year institutions, of every state (excluding DC) from the year of 2004 to 2016. Each row represents a state, and each column corresponds to an academic year. The first 5 rows of the dataset:
...
```{r warning = FALSE, message = FALSE}
avg <- filter(df, State == 'Average') %>% rename(National_avg = State_avg) %>%
  select(-State)
